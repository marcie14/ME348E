...
###############  BELOW IS WHERE COS MATH IS 
			...
			if step == 0: # MIDDLE
				...
				if mini_step == 0: # initiate turn right
					...
					# if len(avg_cos) >= 5:
					# 	avg_cos.pop(0)
					# if (avg_IR < 0.99): # (np.average(avg_cos) > 0.9) or 
					# 	print('has turned 30-60 deg')
					# 	driveAction = 0
					# 	# MODE = 2
					# 	mini_step = 2
				...
				if mini_step == 0: # initiate turn left
					...
					print('turning mid')
					if (front < 10) or ( (avg_IR < 0.8)): #17 < abs(left - right) < 23 or
						driveAction = 0
						mini_step = 2
					# cos = [math.cos(r1 / right)]
					# avg_cos = avg_cos + cos # get degrees has turned
					...
					# 	step = 1
					# 	mini_step = 2
					# elif ((abs(np.average(avg_cos) < 0.6))):
...
# ########################################### END COS MATH			
			...
			# feedAction = 0
			# if (f_prime_switch == 1)
			# if step == 0: # if first time in function
			...
			# 		if mini_step == 0: # if in first section of shoot cycle
			# 			# driveAction = 3 # move straight
			# 			# if ((now - shoot_drive_old) > 2): #if has been driving straight for 2 sec
			# 			driveAction = 0 # stop moving
			# 			mini_step = 1 # enter second mode of shoot cycle

			# 		elif mini_step == 1: # if second mode of shoot cycle
			# 			if shoot_switch == 0: #  if shoot switch has not been pushed
			# 				shootAction = 1 # shoot
			# 				mini_step = 2 # update action of been shot
			# 			if shoot_switch == 1:
			# 				shootAction = 1

			# 		elif mini_step == 2: # if third mode of shoot cycle
			...
			# 				driveAction = 0 # stop moving
			# 				step = 0 # reset variables for MODE 2
			# 				mini_step = 0 # reset variables for MODE 2
		
		elif MODE == 3: ## feed Mode
			driveAction = 0
			if  feedAction == 0 and (f_drop_switch == 0 and f_prime_switch == 0):
				f_drop_switch = 1
			# shootAction = 0
			# if (f_prime_switch == 0) or (f_drop_switch == 0):
			# 	feedAction  = 1
			if avg_IR > 0.9 and (f_drop_switch == 1 or f_prime_switch == 1):
				feedAction = 0
				MODE = 1
